...
export default function AdvancedOCRClient() {
	...
	const _processPdfFile = async (loadPdfJs, worker, file, onProgress) => {
		let pdf;
		try {
	...
	const processFiles = async () => {
		...
		const worker = await deps.createTesseractWorker(selectedLanguage, 1);
		...
		for (const fileData of files) {
			...
			try {
				...
				if (fileData.file.type === 'application/pdf') {
					result = await _processPdfFile(deps.loadPdfJs, worker, fileData.file, onProgress);
					...
					<Card>
						...
						<CardContent className="space-y-4">
							...
							<Button
								...
								disabled={files.length === 0 || isProcessing || isLoadingDependencies}
							>
								{isLoadingDependencies ? 'Loading OCR Engine...' : isProcessing ? 'Processing...' : 'Process Document'}
							</Button>
						</CardContent>
					</Card>
				</div>
