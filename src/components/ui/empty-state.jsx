...
const emptyStateVariants = cva(
  "flex flex-col items-center justify-center text-center p-8 transition-all duration-300",
  {
    variants: {
      variant: {
        default: "bg-background dark:bg-background/50",
        subtle: "bg-transparent",
        card: "bg-background border border-border shadow-sm",
        glass: "bg-background/70 dark:bg-background/70 backdrop-blur-xl border border-white/30 dark:border-border/30",
      },
      size: {
        sm: "max-w-xs",
        default: "max-w-md",
  ...
  return (
    ...
    >
      {Icon && (
        <div 
          className={cn(
            "mb-4 p-3 bg-background dark:bg-background",
            animate && "animate-in zoom-in-50 delay-100 duration-500"
          )}
        >
          <Icon 
            className={cn(
              "h-8 w-8 text-foreground dark:text-foreground",
              iconClassName
            )} 
          />
        </div>
      ...
      {description && (
        <p 
          className={cn(
            "text-sm text-foreground dark:text-foreground mb-4 max-w-sm",
